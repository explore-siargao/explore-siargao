...
const ReportHostModal = ({isOpen, onClose}: ReportHostModalProps) => {
    ...
    const increasePageLevel = useReportHostStore((state) => state.increasePageLevel)
    const decreasePageLevel = useReportHostStore((state) => state.decreasePageLevel)
    const setFirstLevelIndex = useReportHostStore((state) => state.setFirstLevelIndex)
    const setSecondLevelIndex= useReportHostStore((state) => state.setSecondLevelIndex)
    ...
    const setMultipleDescription = useReportHostStore((state) => state.setMultipleDescription)
    const setRemoveDescription = useReportHostStore((state) => state.setRemoveDescription)
    const setClearDescription = useReportHostStore((state) => state.setClearDescription)
    ...
    return (
        <ModalContainer isOpen={isOpen} onClose={onClose} size="sm">
            <div className="h-[60vh] overflow-y-auto">
                    ...
                    <Typography variant="h2" fontWeight="semibold">
                        {
                            ...
                            "We got your report" :
                            // @ts-ignore
                            pageLevel === 3 ? reportArr[firstLevelIndex]?.descriptions[secondLevelIndex]?.displayTitle :
                            "What's happening?"
                        }
                    </Typography>
                    <Typography variant="h4" className={`${isReported && "mt-7"}`}>
                        {
                            ...
                            "help keep the " + APP_NAME +" community safe and secure." :
                            // @ts-ignore
                            pageLevel === 3 && reportArr[firstLevelIndex]?.descriptions[secondLevelIndex]?.subtitle ? reportArr[firstLevelIndex]?.descriptions[secondLevelIndex]?.subtitle 
                            :
                ...
                {!isReported &&
                    <fieldset className="px-5">
                        <div className={`${
                                pageLevel != 3 ? "divide-y divide-gray-200 border-gray-200" : 
                                pageLevel === 3 && firstLevelIndex === 0 && secondLevelIndex <= 2 ? "grid grid-cols-2 gap-3" 
                                : "mt-4"}`
                            }>
                        {
                            // third level
                            pageLevel === 3 ?
                                firstLevelIndex === 0 && secondLevelIndex <= 2 ? 
                                    // @ts-ignore
                                    (reportArr[firstLevelIndex]?.descriptions[secondLevelIndex]?.descList.map((report, index) => (
                                        <label 
                            ...
                            // second level
                            pageLevel === 2 ?
                            (reportArr[firstLevelIndex]?.descriptions.map((report, index) => (
                                <div key={`second-level-con-${index}`} className="relative flex items-start py-5">
                                    <div className="min-w-0 flex-1 leading-6">
                                    <label htmlFor={`second-level-${index}`} className="select-none">
                                        {report.reason}
                                    </label>
                            ...
                            (reportArr.map((report, index) => (
                                <div key={`first-level-con-${index}`} className="relative flex items-start py-5">
                                    <div className="min-w-0 flex-1 leading-6">
                                    <label htmlFor={`first-level-${index}`} className="select-none">
                                        {report.name}
                                    </label>
                                    </div>
                                    <div className="ml-3 flex h-6 items-center">
            ...
            <div className={`${pageLevel > 1 && !isReported ? "justify-between" : "justify-end"} border-t border-gray-300 flex items-center`}>
                {pageLevel > 1 && !isReported && <Button variant="ghost" className="ml-1 underline font-medium" onClick={() => {
                    decreasePageLevel()
                    setClearDescription()
                    }
                    }>Back</Button>}
                <Button variant="default" className="mx-5 my-4 px-8  disabled:opacity-40" 
                    onClick={() =>
                       { 
                            isReported ? 
                            onClose()
                            :
                            // @ts-ignore
                            (pageLevel === 2 && reportArr[firstLevelIndex]?.descriptions[secondLevelIndex]?.readyToSubmit) || pageLevel === 3 ?
                            setIsReported() 
                            : 
                            increasePageLevel()
                        }
                ...
                >
                    {isReported ? "Ok" :pageLevel === 3 ? "Submit" : pageLevel === 2 ? "Next" : "Ok"}
                </Button>
            </div>
        </ModalContainer>
    )
