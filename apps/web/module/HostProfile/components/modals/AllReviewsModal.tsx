...
const AllReviewsModal = ({isOpen, onClose, reviews}: AllReviewsModalProps) => {
    return (
        <ModalContainer isOpen={isOpen} onClose={onClose} size="sm">
            <div className="p-5 h-[80vh] overflow-y-auto">
                <Typography variant="h2" fontWeight="semibold">60 reviews</Typography>
                <div>
                ...
                {reviews.map((data, index) => (
                    <div key={index} className="my-5">
                        <div className="flex space-x-2">
                            <div className="w-4/5">
                                <Typography variant="h3" fontWeight="semibold" >Seaview Villa with 3 queen beds</Typography>
                            </div>
                            <div className="w-1/5 bg-gray-200 h-14 relative rounded-md">
                                <Image src={`${ASSET_ROOT}/1.jpg`} layout="fill" objectFit="cover" className="rounded-md" alt="listing-image"/>
                            </div>
                        </div>
                        <ReviewCard 
                ...
                <Button variant="outline" className="text-base font-semibold">Show more reviews</Button>
            </div>   
        </ModalContainer>
    )
}
