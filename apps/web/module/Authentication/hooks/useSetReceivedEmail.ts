...
export async function setReceivedEmail(userId: number, canReceive: boolean) {
  ...
  return await apiService.patch(`${API_URL_USERS}/${userId}/received-email`, { canReceive })
}
function useSetReceivedEmail(
  ...
  const query = useMutation({
    mutationFn: ({ canReceive }: { canReceive: boolean }) => setReceivedEmail(userId, canReceive),
