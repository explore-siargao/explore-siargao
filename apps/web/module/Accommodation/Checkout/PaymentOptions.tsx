...
export default function PaymentOptions() {
  ...
  const defaultSelectedOption = combinedOptions.find((option) => option.selected)
  useEffect(() => {
              ...
              <RadioGroup.Option
                ...
                className={({ checked }) =>
                  cn(
                    ...
                    settingIdx === combinedOptions.length - 1 ? 'rounded-bl-md rounded-br-md' : '',
                ...
                {({ active, checked }) => (
                  <>
                    ...
                    <span className="ml-3 flex flex-col w-full">
                      <RadioGroup.Label
                        as="span"
                        className={cn(checked ? 'text-primary-700' : 'text-text-900', 'block text-sm font-medium')}
                      >
                        {setting.name}
                      </RadioGroup.Label>
                      <RadioGroup.Description
                        as="span"
                        className={cn(checked ? 'text-primary-700' : 'text-text-300', 'block text-sm')}
                      >
