  ...
  return (
    <div className="border rounded-xl shadow-lg px-6 pb-6 pt-5 flex flex-col divide-text-100 overflow-y-auto mb-5">
      ...
      <div className="font-semibold grid grid-cols-1 gap-3 w-full">
        ...
        <Button
          variant="primary"
          onClick={() => router.push(`/accommodation/1/checkout?guests=${totalGuest}&dateFrom=${dateRange.from ? format(dateRange.from, "MM/dd/yyyy") : ''}&dateTo=${dateRange.to ? format(dateRange.to, "MM/dd/yyyy") : ''}&adultCount=${adults}&childrenCount=${children}&infantCount=${infants}`)}
