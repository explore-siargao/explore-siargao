...
export async function addDescription(userID: number ,listingId: number | undefined, props: T_ListingDescription) {
...
function useAddListingDescription(userID: number, listingId: number) {
  const query = useMutation({
    mutationFn: (props: T_ListingDescription) => addDescription(userID, listingId, props),
  })
  return query
}
