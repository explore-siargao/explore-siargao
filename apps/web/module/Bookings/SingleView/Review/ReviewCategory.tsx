...
const ReviewCategory = ({title, rating, isHorizontal, icon: Icon}: ReviewCategoryProps) => {
    if(isHorizontal) {
        return (
            <div>
                <div className="flex justify-between items-center w-full lg:px-4 py-4">
                    ...
                    <Typography variant={"h5"} className="font-semibold">{rating}</Typography>
                </div>
            </div>
        )
    }
        ...
        return (
            ...
            <Typography variant={"h5"} className="font-bold">{title}</Typography>
            <Typography variant={"h4"} className="font-bold mb-5">{rating}</Typography>
