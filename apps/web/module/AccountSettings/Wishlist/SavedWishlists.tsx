...
const SavedWishlists = ({itemId, listingId, location, date, distance, price, imageKey, isNight, ratings, note, showAddNote}: SavedWishlistsProps) => {
    ...
    return (
        <div key={itemId}>
            ...
            />
            <div className={`${note && "bg-gray-100 px-3 py-2"} w-full mt-2 rounded-lg`}>
            {note === null ? (
                <AddEditNoteButton
                onClick={showAddNote}
                id={"addNoteBtn" + itemId}
                />
            ) : (
                note + " "
            )}
