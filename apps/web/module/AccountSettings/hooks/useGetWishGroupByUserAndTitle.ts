...
export async function getWishGroupByTitle(userId: number | undefined,title: string) {
  const apiService = new ApiService()
  return await apiService.get(`${API_URL_BOOKINGS}/${userId}/${title}/wishes`)
}

function useGetWishGroupByUserAndTitle(userId: number | undefined, title:string) {
