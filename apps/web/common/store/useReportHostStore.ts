...
const useReportHostStore = create<T_ReportHostState >((set) => ({
    ...
    setFirstLevel: (value) => set((state) => ({
        data: {
            ...state.data,
            name: value
        }
    })),
    setSecondLevel: (value) => set((state) => ({
        data: {
            ...state.data,
            reason: value
        }
    })),
    setDescription: (value) => set((state) => ({
        data: {
            ...state.data,
            description: value
        }
    })),
    setMultipleDescription: (value) => set((state) => ({
        data: {
            ...state.data,
            description: state.data.description ? state.data.description + ', ' + value : value
        }
    })),
    setRemoveDescription: (value) => set((state) => ({
        data: {
            ...state.data,
    ...
    setClearDescription: () => set((state) => ({
        data: {
            ...state.data,
            description: ""
        }
    })),
    setIsReported: () => set((state) => ({
