      ...
      <WidthWrapper width={contentWidth}>
        <div className="md:flex border-b pb-4 md:pb-11">
          <div className="md:flex-1 pb-4 md:pb-0 md:mr-4 border-b md:border-b-0">
            <Typography variant="p" fontWeight="semibold" className="mb-2 text-sm">
              Support
            </Typography>
            <ul>
              {SUPPORT.map((item, index) => (
                <li key={`${item.name}-${index}`}>
                  <Link href={item.link}>
                    <Typography variant="p" className="mb-1 hover:underline inline-block hover:duration-300 cursor-pointer text-sm text-text-500">
                      {item.name}
                    </Typography>
                  </Link>
                </li>
