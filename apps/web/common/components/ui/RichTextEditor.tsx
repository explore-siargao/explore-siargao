...
const RichTextEditor = () => {
  ...
  const toggleList = (type: string) => {
    ...
    if (isActive) {
      ...
      Transforms.wrapNodes(editor, {
        type: type === "ordered-list" ? "ordered-list-item" : "list-item",
        children: [],
      })
    }
  }
  ...
  const Toolbar = () => (
    <div className="flex gap-2 border-1 shadow-md my-4 pl-2 h-10 w-60">
      <button onMouseDown={() => toggleMark("bold")}>
        <Bold size="15px" />
      </button>
      <button onMouseDown={() => toggleMark("italic")}>
        <ItalicIcon size="15px" />
      </button>
      <button onMouseDown={() => toggleList("list")}>
        <List size="17px" />
      </button>
      <button onMouseDown={() => toggleList("ordered-list")}>
  ...
  const ListItemElement = ({
    ...
    <li {...attributes} style={{ listStyleType: "disc", marginLeft: "1em" }}>
      {children}
    </li>
  )
