...
export const getAllHouseRulesByListing = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        if(!getListing){
            return res.json(response.error({message:"This listing is not found on our system"}))
        }
...
export const deleteHouseRule = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        if(!getHouseRule){
            return res.json(response.error({message:"House Rule data already deleted or not found"}))
        }
