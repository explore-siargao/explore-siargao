...
export const getReport = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        if(getReportById){
            ...
            res.json(response.error({message:"Report already deleted or report data not found"}))
        }
    } catch (err:any) {
        res.json(response.error({message:err.message}))
    }
...
export const addReport = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        if(!isInputValid.success){
            return res.json(response.error({message:JSON.parse(isInputValid.error.message)}))
        }
        const getListing = await prisma.listing.findUnique({
            where:{
                id:listingId
        ...
        if(!getListing){
            return res.json(response.error({message:"The listing you are trying to report was not found"}))
        }
...
try {
    ...
    if(!getReport){
        return res.json(response.error({message:"Report that you are trying to update was not found"}))
    }
