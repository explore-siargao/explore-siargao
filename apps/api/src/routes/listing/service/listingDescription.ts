...
export const getDescriptionByListing = async(req:Request, res:Response)=>{
    ...
    try {
        const getDescriptionByListingId = await prisma.listingDescription.findUnique({
            where:{
                id:listingId
            }
        })
...
export const updateDescription = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        if(!getDescription){
            return res.json(response.error({message:"Listing description not found"}))
        }
        if(generalDescription && (aboutSpace || aboutGuestAccess || otherThingsNote)){
