...
export const getAllCancellationPolicies = async(req:Request, res:Response)=>{
try {
...
export const getCancellationPoliciesByListing = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        const getAllCancellationPoliciesByListingId = await prisma.cancellationPolicy.findMany({
            where:{
                listingId:listingId
            }
        })
...
export const getCancellationPolicy = async(req:Request, res:Response)=>{
    ...
    try {
        ...
        if(getCancellationPolicyById){
            ...
            res.json(response.error({message:"Cancellation Policy data not found"}))
        }
    } catch (err:any) {
        res.json(response.error({message:err.message}))
    }
    ...
    try {
        if(title || cancelationDueDate){
            ...
            if(!getCancellationPolicy){
               return res.json(response.error({message:"Cancellation Policy data not found"}))
            }
            const updateCancellationPolicyById = await prisma.cancellationPolicy.update({
                where:{
                    id:id
                },
                data:{
