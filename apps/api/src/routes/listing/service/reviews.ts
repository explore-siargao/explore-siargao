    ...
    if (listing) {
      ...
      const transformedReviews = reviews.map((review) => {
        ...
        return {
          ...
          rating: (review.cleanLinessRates + review.accuracyRates + review.checkInRates + review.communicationRates + review.locationRates + review.valueRates) / 6,
      ...
      if (reviews.length !== 0) {
        res.json(
          response.success({
            ...
            overallRating:(cleanlinessAverage + accuracyAverage +checkInAverage + communicationAverage + locationAverage +valueAverage)/6,
