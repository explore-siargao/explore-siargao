...
export const getPaymentHistoryTable = async(req: Request, res: Response) => {
    ...
    const completedDate = new Date(dateType as string).toLocaleDateString("en-US", { month: "2-digit", day: "2-digit", year: "numeric" });
    ...
    const getCompletedStatus = earnings.filter((item) => {
        ...
        if (dateType === "all") {
            ...
            return item.status === "Completed" && (dateType.toLocaleLowerCase() === monthYear.toLocaleLowerCase());
        }
        return false
    });
    ...
    const getCancelledStatus = earnings.filter((item) => {
        ...
        if (dateType === "all") {
            ...
            return item.status === "Cancelled" && (dateType.toLocaleLowerCase() === monthYear.toLocaleLowerCase());
        }
    ...
    // sums
    const sumOfCompleted = getCompletedStatus.reduce((sum, item) => sum + (item?.earning as number), 0);
    const sumOfCancelled = getCancelledStatus.reduce((sum, item) => sum + (item?.earning as number), 0);
