...
filteredBooking.sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());

    const totalEarnings = filteredBooking.reduce((sum, item) => sum + (item?.earning as number), 0);
